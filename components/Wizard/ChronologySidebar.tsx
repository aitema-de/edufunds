...
export function ChronologySidebar({
  ...
  return (
    <aside className="flex flex-col gap-6">
      <section className="rounded-xl border border-[#1c1917]/10 bg-white/80 p-4">
              ...
              return (
                <li key={m.id} className="group">
                  <div className="flex items-center justify-between gap-2">
                    <div
                      className={
                        m.role === "ai" ? "text-slate-600" : "text-[#57534e] font-medium"
                      }
                    >
                      {m.role === "ai" ? "KI" : "Sie"}
                    </div>
                    {canEdit && !isEditing && (
                      <button
                        ...
                        className="rounded p-1 text-slate-500 opacity-0 transition hover:bg-slate-100 hover:text-[#57534e] group-hover:opacity-100"
                    ...
                    <div className="mt-1">
                      ...
                      <div className="mt-1.5 flex items-center justify-end gap-2">
                        <button
                          ...
                          onClick={cancelEdit}
                          disabled={editBusy}
                          className="inline-flex items-center gap-1 rounded border border-[#1c1917]/15 px-2 py-1 text-xs text-slate-700 hover:bg-slate-100"
                        >
                          <X className="h-3 w-3" />
                          Abbrechen
                        </button>
