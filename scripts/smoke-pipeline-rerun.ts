/**
 * End-to-End-Re-Run der Pipeline mit den gleichen UAT-Inputs vom 28.04.
 *
 * Laedt Session 37571430-... aus Dev-DB, ruft runPipeline mit programm/facts/
 * richtlinie/messages auf und prueft den finalen Antragstext + Finanzplan auf
 * die 8 Halluzinations-Marker, die vorher im UAT-Output drinstanden.
 *
 * Validiert, ob der Bug-#1-Fix (geschaerfter Critique) ueber Recheck und
 * Revision tatsaechlich beim finalen Output ankommt — oder ob die Section-
 * Stage die Halluzinationen weiter produziert (das waere Bug #2).
 *
 * Run: `npx tsx --env-file=.env.local scripts/smoke-pipeline-rerun.ts`
 */
...
async function main() {
  ...
  // End-to-End-Re-Run mit geschaerftem Critique:
  ...
  await writeFile(resolve(outDir, "pipeline-rerun-finaltext.md"), result.artefacts.finalText ?? "");
  await writeFile(resolve(outDir, "pipeline-rerun-finanzplan.json"), newFinanzplanText);
  await writeFile(
    resolve(outDir, "pipeline-rerun-critique.json"),
    JSON.stringify({ findings, resolutions }, null, 2)
  );
  console.log(`\nOutputs in ${outDir}/pipeline-rerun-*.{md,json}`);
}
