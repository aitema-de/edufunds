  ...
  const res = await query<DbRow>(
    `UPDATE ki_antraege
       SET status = 'paid',
           ...
           -- Antrag automatisch an die Käufer-E-Mail binden, damit er ohne
           -- separaten Magic-Link-Opt-in geräteübergreifend unter "Meine Anträge"
           -- auftaucht. lower+trim = identisch zu normalizeEmail() (identity.ts),
           -- damit listSessionsByEmail() exakt matcht. Eine bereits explizit
           -- gesetzte author_email (Magic-Link zuvor) wird NICHT überschrieben.
