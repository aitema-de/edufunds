...
describe("zahlKern", () => {
   ...
   */
  it("löst den deutschen Tausenderpunkt auf", () => {
  ...
  it("lässt den JSON-Dezimalpunkt unangetastet", () => {
  ...
  it("gibt null zurück, wenn keine Ziffer da ist", () => {
...
describe("Ist-Zahlen zum Bedarf", () => {
  it("ist offen, wenn die Bedarfsfelder keine Zahl tragen", () => {
    const f: WizardFacts = {
      schule: { besonderheiten: "viele Kinder mit Förderbedarf" },
      projekt: { kurzbeschreibung: "Wir wollen die Leseförderung ausbauen" },
    };
    expect(status(f, "bedarf-ist-zahlen")).toBe("offen");
  });

  it("wird erfüllt, sobald zwei Bedarfsfelder Zahlen tragen", () => {
    const f: WizardFacts = {
  ...
  it("zählt die Schülerzahl NICHT als Bedarfsbeleg — sie ist die Größe der Schule", () => {
    const f: WizardFacts = {
...
describe("Kosten und Mengen je Posten", () => {
  ...
  it("ist nur teilweise erfüllt, wenn nur die Gesamtsumme feststeht", () => {
    const f: WizardFacts = {
  ...
  it("ist erfüllt, wenn die Mehrheit der Posten beziffert ist", () => {
    const f: WizardFacts = {
  ...
  it("gilt als geklärt, wenn der Nutzer die Kostenfrage selbst als offen benannt hat", () => {
    const f: WizardFacts = {
      programmpassung: { offene_luecken: ["Konkrete Budgethöhe noch offen"] },
    };
...
describe("Wer und Wann im Arbeitsplan", () => {
  ...
  it("ist erfüllt mit Zuständigkeit UND Zeitbezug", () => {
    const f: WizardFacts = {
      projekt: {
        aktivitaeten: ["Kleingruppen durch zwei Lehrkräfte ab dem zweiten Halbjahr"],
...
describe("Ausgangswert und Zielwert je Indikator", () => {
  it("ist nur teilweise erfüllt, wenn der Indikator eine einzelne Zahl nennt", () => {
  ...
  it("ist erfüllt bei Ausgangs- UND Zielwert", () => {
    const f: WizardFacts = {
...
describe("Beschlüsse und Zusagen des Trägers", () => {
  it("ist offen ohne jeden Trägerbezug", () => {
    const f: WizardFacts = { wirkung: { nachhaltigkeit: "Wir führen das Angebot fort." } };
    expect(status(f, "traeger-zusage")).toBe("offen");
  });

  it("ist erfüllt bei benannter Vereinbarung in der Nachhaltigkeit", () => {
    const f: WizardFacts = {
  ...
  it("gilt als geklärt, wenn der Nutzer den Beschluss ausdrücklich verneint hat", () => {
    const f: WizardFacts = {
...
describe("tiefeQuote", () => {
  ...
  it("lässt geklärte Punkte aus dem Nenner fallen — eine ehrliche Fehlanzeige darf nicht drücken", () => {
  ...
  it("gibt null zurück, wenn ALLE Punkte geklärt sind (nichts Messbares)", () => {
    expect(
